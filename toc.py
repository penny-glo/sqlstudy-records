...
def creat_directory(f):
    i = 0
    directory = []
    directory.append('<a name="目录">**目录**</a>\n')
    for line in f:
    ...
    for j in range(length):
        ...
        if splitedline[0] in headline:
            ...
            if j == length - 1 and lines_in_file[j][-1] != '\n':
                ...
                lines_in_file[j] = lines_in_file[j].replace(splitedline[0] + ' ',splitedline[0] + ' ' + '<a name="' + str(i) + '">')[:] + '</a><a style="float:right;text-decoration:none;" href="#目录">[Top]</a>' + "\n"
                i = i + 1
            else:
                directory.append(creat_directory_line(lines_in_file[j],splitedline[0],i))
                lines_in_file[j] = lines_in_file[j].replace(splitedline[0] + ' ',splitedline[0] + ' ' + '<a name="' + str(i) + '">')[:-1] + '</a><a style="float:right;text-decoration:none;" href="#目录">[Top]</a>' + "\n"
                i = i + 1
    return directory
